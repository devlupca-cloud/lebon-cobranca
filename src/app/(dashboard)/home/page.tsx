...
function formatChangeLabel(percent: number | null): string {
  if (percent == null) return 'Sem dado do mês anterior para comparar'
  const signal = percent >= 0 ? '+' : ''
  return `${signal}${percent.toFixed(0)}% em relação ao mês anterior`
}
      ...
      <main className="min-h-full bg-[#f1f4f8]">
        <div className="mx-auto max-w-6xl px-6 py-8">
          ...
          <p className="mt-2 text-amber-600">
            Configure sua empresa (company_users) para acessar o dashboard.
          </p>
        </div>
      </main>
    )
    ...
    ? [
        {
          ...
          change: formatChangeLabel(stats.customersChangePercent),
          hasComparison: stats.customersChangePercent != null,
          icon: KPI_ICONS.customers,
        },
        {
          title: 'Contratos Ativos',
          value: stats.activeContracts.toLocaleString('pt-BR'),
          change: formatChangeLabel(stats.activeContractsChangePercent),
          hasComparison: stats.activeContractsChangePercent != null,
          icon: KPI_ICONS.contracts,
        },
        {
          title: 'Valor Total',
          value: formatCurrency(stats.totalValue),
          change: formatChangeLabel(stats.totalValueChangePercent),
          hasComparison: stats.totalValueChangePercent != null,
          icon: KPI_ICONS.value,
        },
        {
          title: 'Novos Este Mês',
          value: stats.newThisMonth.toLocaleString('pt-BR'),
          change: formatChangeLabel(stats.newThisMonthChangePercent),
          hasComparison: stats.newThisMonthChangePercent != null,
          icon: KPI_ICONS.new,
        },
      ]
    : []
        ...
        <section className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {loading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="rounded-[8px] border border-slate-200/80 bg-white p-5 shadow-sm"
              >
